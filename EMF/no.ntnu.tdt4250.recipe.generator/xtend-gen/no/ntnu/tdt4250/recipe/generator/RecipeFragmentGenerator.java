...
public class RecipeFragmentGenerator {
  ...
  public boolean hasAnySwitches(final Page page) {
    ...
    for (final EObject child : _eContents) {
      ...
      if (!_matched) {
        if (child instanceof Nutrients) {
          _matched=true;
          PercentageAmountToggle _percentagetoggle = ((Nutrients)child).getPercentagetoggle();
  ...
  public CharSequence generateBindings(final Page page) {
    ...
    _builder.newLineIfNotEmpty();
    {
      ...
      for(final Section child : _sortBy) {
        CharSequence _generateSectionBindings = this.generateSectionBindings(child);
  ...
  public CharSequence generateSetupSwitches(final Page page) {
    ...
    _builder.newLine();
    {
      ...
      Iterable<Section> _filter = IterableExtensions.<Section>filter(Iterables.<Section>filter(page.eContents(), Section.class), _function);
      for(final Section child : _filter) {
        _builder.append("   ");
        CharSequence _generateSwitchSetup = this.generateSwitchSetup(child);
