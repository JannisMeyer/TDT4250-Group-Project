...
class RecipeFragmentGenerator {
    ...
    def hasAnySwitches(Page page) {
        for (section : page.section) {
            switch section {
                Ingredients: if (section.unittoggle !== null) return true
                Nutrients: if (section.percentagetoggle !== null) return true
            }
        }
        return false
    }
    ...
    def generateBindings(Page page) '''
		...
		«FOR section : page.section.sortBy[orderNumber]»
			«generateSectionBindings(section)»
		«ENDFOR»
	'''
    ...
    def generateSetupSwitches(Page page) '''
		private fun setupSwitches() {
		   «FOR section : page.section.filter[s | hasSwitches(s)]»
		   	«generateSwitchSetup(section)»
		   «ENDFOR»
		}
	'''
